...
//	-E_NO_FREE_ENV i fall NENVS environments are allocated
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	int init = ROUNDDOWN((int)va,PGSIZE);
	int fin = ROUNDUP((int)va + (int)len, PGSIZE);
	int i=0;
	for (i=init;i<fin;i++)
	{
		struct PageInfo *p;
		p = page_alloc(0);
		if (!p)
			panic("Error happened in region alloc");
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	// LAB 3: Your code here.
	struct Elf* elfsectionheader;
	elfsectionheader= (struct Elf*) binary;
	struct Proghdr *l1; 

	l1 = (struct Proghdr *) ((uint8_t *) elfsectionheader + elfsectionheader->e_phoff);
	struct Proghdr *l2;
	l2 = l1 + elfsectionheader->e_phnum;
	// eph = ph + ELFHDR->e_phnum;
	
	lcr3(PADDR(e->env_pgdir));

	for ( ;l1 < l2; l1++) {
		if (l1->p_type != ELF_PROG_LOAD) 
			continue;
		region_alloc(e, (void *) l1->p_va, l1->p_memsz);
		memmove((void *) l1->p_va, binary+l1->p_offset, l1->p_filesz);
		memset((void *) l1->p_va + l1->p_filesz, 0, (l1->p_memsz - l1->p_filesz));
	}

	e->env_tf.tf_eip = elfsectionheader->e_entry;
...
void
env_replace(uint8_t *binary, enum EnvType type, int argc, char** argobt)
{
	// destory curenv
	// env alloc with parent id
	// set id
	// lcr3 e->pgdir
	// e->tf_esp pe write
	// lcr3 kern
	...
	// env run
	// cprintf("%08x\n",kern_pgdir);
	// cprintf("argobtained: %s\n",argobt[0]);


	load_icode(curenv,binary);
	// cprintf("argobtained post : %s\n",argobt[0]);
	uint32_t *ust = (uint32_t *) curenv->env_tf.tf_esp;
	...
	curenv->env_tf.tf_esp = (uintptr_t) ust;

	curenv->env_type = type;
	// lcr3(PADDR(kern_pgdir));
	// cprintf("Leaving replace functon\n");
	// env_run(curenv);
}
